...
public class BlockAction : MonoBehaviour
{
    ...
    [Header("ExtraChallenge")]
    [SerializeField] HeightLimitationChallenge hlc;
    ...
    private void OnCollisionStay(Collision collision)
    {
        ...
        {
            //pivotObj.transform.parent = collision.gameObject.transform.root;
            //Rigidbody rb = pivotObj.transform.parent.GetComponent<Rigidbody>();
            ////List<Vector3> weightCubePosition = new List<Vector3>();
            ////foreach (GameObject obj in pivotObj.transform.parent) if (obj.GetComponent<BlockWeight>() != null) weightCubePosition.Add(obj.transform.position);
            ////Vector3 sumOffset = Vector3.zero;
            ////foreach (var offset in weightCubePosition) sumOffset += offset;
            ////rb.centerOfMass = rb.position + (sumOffset / weightCubePosition.Count);
            //rb.mass += .2f;
            //pivotObj = null;
        ...
        //if (toRb.GetComponent<BlockColor>().blockColor != collision.gameObject.GetComponent<BlockColor>().blockColor) return;
        //Rigidbody fromRb = collision.transform.parent ? collision.transform.parent.GetComponent<Rigidbody>() : collision.gameObject.GetComponent<Rigidbody>();
        //toRb.interpolation = RigidbodyInterpolation.Interpolate;
        //HingeJoint hinge = toRb.gameObject.AddComponent<HingeJoint>();
        //hinge.connectedBody = fromRb;
    ...
    {
        if ((flagStatus & FlagsStatus.PressDownButton) == FlagsStatus.PressDownButton || (flagStatus & FlagsStatus.Collapse) == FlagsStatus.Collapse) return;
    ...
    void HandleNewBlockSpawn()
    {
        ...
        if (timer > 0.5f && (flagStatus & FlagsStatus.Collapse) != FlagsStatus.Collapse)
        {
            int tempIndex = 0;
